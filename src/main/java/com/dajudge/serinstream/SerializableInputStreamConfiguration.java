...
/**
 * A singleton class to configure serialization used by {@link SerializableInputStream}. The default configuration
 * serialized to a file in the user's temp directory, imposes no limit on the upload size and does not use encryption.
 * 
...
public class SerializableInputStreamConfiguration {
    ...
    /**
     * Sets the serialization temp store. The temp store is used for persisting serialization data while the stream is
     * retrieved. By default the user's temporary directory is used for storing the data in order to avoid out of memory
     * situations.
     * 
     * @param tempStore the temp store the be used for deserialization.
     */
